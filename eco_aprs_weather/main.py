...
def telemtry_analog_set(field, value):
    ...
    """
    value = "%g" % float(value)
    if value == telemetry.analog[int(field)]:
      drift = 'NOCHANGE'
    else:
      drift = 'CHANGED'
    telemetry.analog[int(field)] = value
    status = {
      'bit_status': drift,
      'value': value
    }
...
def wxnow():
    ...
    if singleton.weather.get('dateutc'):
        ...
        if duration_in_s >= configuration.stale_threshold:
            ...
            error = f'[WxTempOffAir:ERROR:No New Data from GW since {dateutc}UTC ({t} seconds)] {configuration.status}'
            winddir = 0
            windspeedmph = 0
            windgustmph = 0
            hourlyrainin = 0
        ...
        #date = datetime.datetime.utcnow().strftime("%b %d %Y %H:%M\n")
        error = f'[WxTempOffAir:WARN:Waiting for ECOWITT GW data stream] {configuration.status}'
        winddir = 0
        windspeedmph = 0
        windgustmph = 0
        hourlyrainin = 0
