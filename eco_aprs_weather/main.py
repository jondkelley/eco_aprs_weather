...
def weather_report():
    ...
    # example of what device sends: curl -d "PASSKEY=XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX&stationtype=GW1000B_V1.5.5&dateutc=2020-01-05+02:25:46&tempinf=73.9&humidityin=42&baromrelin=2humidity1=37&temp2f=46.58&humidity2=49&soilmoisture1=93&soilmoisture2=52&batt1=0&batt2=0&soilbatt1=1.8&soilbatt2=1.7&freq=915M&model=GW1000_Pro" -H "Content-Type: application/x-wwwlhost:5001/data/report
...
def metrics(metric):
    ...
    for day, v in wx.metrics.items():
        for hour, item in v.items():
            ...
            try:
                output.append({"x": epoch, "y": float(item[metric])})
            except ValueError:
                output.append({"x": epoch, "y": int(item[metric])})
